    ...
    case "POST":
      {
        ...
        const registeredReferred = (
          await Award.findAll({ where: { referringId: body.user.id } }) 
        ...
        const milestone10Transferred = (
          await Award.findAll({ where: { milestoneId: milestoneId,winnerId:body.user.id } })
        ).length;
        if (milestone10Transferred < Math.floor(registeredReferred / objetivo)) {
        ...
        if (milestone50Transferred < Math.floor(registeredReferred / objetivo)) {
        ...
        if (milestone100Transferred < Math.floor(registeredReferred / objetivo)) {
        ...
        const pendingTokens = await Award.findAll({
          where: { transferred: false, winnerId: body.user.id },
        });
